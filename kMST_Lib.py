...
def write_in_files(mdl, run_log, k, f):

    write_log(mdl, run_log, k, f)
    write_log(mdl, 'solution_log.txt', k, f)

    cwd = os.getcwd()
    if k == 2 and f == 'g01.dat':
        mdl.export_as_lp(cwd)

    with get_environment().get_output_stream("solution_" + mdl.name + "_" + f[:3]
                                             + "_k_" + str(k) + ".json") as fp:
        mdl.solution.export(fp, "json")


def update_table(table, mdl, k, f):
    # table = [['formulation', 'graph', 'k', 'objective_value', 'time', 'gap', 'status',
    ...
    table.append([mdl.name, f[:3], k, mdl.objective_value, details.time, details.mip_relative_gap, details.status,
